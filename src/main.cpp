...
static const int MODIFIER_INTERVAL_SWITCH = 0; // pos start block

int64 nChainStartTime = 1408199514;
...
const string strMessageMagic = "MiracleCoin Signed Message:\n";
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++)
    {
        ...
        // enforce only non-coinbase transactions to allow 0 pow amount.
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in MiracleCoin
    bool fStrictPayToScriptHash = true; // Always active in MiracleCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MiracleCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Man dies after 35 found in container";
        CTransaction txNew;
        txNew.nTime = nChainStartTime;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 86062 << CBigNum(83802) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        ...
        assert(block.hashMerkleRoot == uint256("0x1f2791eebf2e5a45a81a8161ca9f198eb25af2f7eafc9dae224ec0f7bf881dc8"));
...
unsigned char pchMessageStart[4] = { 0x9a, 0xa8, 0xa4, 0xf5 };
